...
const defaultPayload = {
  ...
  author: { name: '', bio: '' },
  experience: { title: '', short: '', format: 'read', minutes: 3, paragraphs: [], question: '', audioUrl: '', audioEmbedUrl: '' },
...
function App() {
  ...
  async function refresh() {
    async function everyPage(path, field) {
      const result = [];
      for (let offset = 0; offset < 10000; offset += 100) {
        const batch = (await request(`${path}${path.includes('?') ? '&' : '?'}limit=100&offset=${offset}`))[field];
        result.push(...batch);
    ...
    const [list, all, library] = await Promise.all([
      request(`/v1/admin/items?kind=${kind}&status=${status}&q=${encodeURIComponent(query)}&limit=50&offset=${pageIndex * 50}`),
      everyPage('/v1/admin/items', 'items'),
      everyPage('/v1/admin/media', 'media'),
    ]);
    setItems(list.items);
    setHasNextPage(list.items.length === 50);
    setAllItems(all);
    setMedia(library);
  }
  useEffect(() => { setPageIndex(0); }, [kind, query, status]);
  useEffect(() => { if (user) refresh().catch(error => setMessage(error.message)); }, [user, kind, query, status, pageIndex]);
  ...
  async function uploadFile(file, onReady) {
    if (!file) return;
    setBusy(true); setMessage('در حال بارگذاری فایل…');
    try {
      const uploaded = await upload(`nahanja/${crypto.randomUUID()}-${file.name.toLowerCase().replace(/[^a-z0-9._-]/g, '-')}`, file, {
        access: 'public',
      ...
      for (let attempt = 0; attempt < 12; attempt++) {
        const library = [];
        for (let offset = 0; offset < 10000; offset += 100) {
          const batch = (await request(`/v1/admin/media?limit=100&offset=${offset}`)).media;
          library.push(...batch);
          if (batch.length < 100) break;
        }
        asset = library.find(row => row.public_url === uploaded.url);
        if (asset) { setMedia(library); break; }
        await new Promise(resolve => setTimeout(resolve, 1000));
      }
      if (asset && onReady) onReady(asset);
      setMessage(asset ? 'فایل بارگذاری و به محتوا متصل شد؛ پیش‌نویس را ذخیره کنید.' : 'فایل بارگذاری شد؛ ثبت آن کمی زمان می‌برد. کتابخانهٔ مدیا را تازه کنید.');
      if (!onReady) await refresh();
    } catch (error) { setMessage(error.message); }
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      ...
      {tab === 'media' ? <section className="content-pad"><div className="page-heading"><div><div className="eyebrow">MEDIA LIBRARY</div><h1>کتابخانهٔ مدیا</h1><p>تصویر جلد، عکس، صوت و ویدئو را اینجا بارگذاری و به محتوا متصل کنید.</p></div><label className="button primary upload-button">بارگذاری فایل<input type="file" accept="image/jpeg,image/png,image/webp,audio/mpeg,audio/mp4,audio/wav,video/mp4,video/webm" onChange={event => uploadFile(event.target.files?.[0])} /></label></div>{uploadProgress > 0 && <div className="progress"><span style={{ width: `${uploadProgress}%` }} /></div>}<button className="text-button" onClick={() => refresh()}>تازه‌سازی فهرست</button><div className="media-grid">{media.map(asset => <MediaCard asset={asset} key={asset.id} onSave={(alt, caption) => mutate(`/v1/admin/media/${asset.id}`, { method: 'PUT', body: JSON.stringify({ alt, caption }) })} />)}</div></section> : <div className="content-grid">
        <section className="list-pane"><div className="page-heading"><div><div className="eyebrow">CONTENT COLLECTION</div><h1>{labels[kind] || kind}</h1><p>{items.length} مورد در صفحهٔ {pageIndex + 1}</p></div><button className="button primary" onClick={newItem}>افزودن</button></div><div className="filters"><input placeholder="جست‌وجوی محتوا…" value={query} onChange={event => setQuery(event.target.value)} /><select value={status} onChange={event => setStatus(event.target.value)}><option value="all">همه</option><option value="published">منتشرشده</option><option value="draft">پیش‌نویس</option><option value="archived">آرشیو</option><option value="trash">زباله‌دان</option></select></div><div className="item-list">{items.map(item => <button key={item.id} className={`item-row ${selected === item.id ? 'selected' : ''}`} onClick={() => openItem(item.id)}><div className="item-title">{title(item)}</div><div className="item-meta"><span>{item.slug || item.stable_key}</span><span className={`status ${item.deleted_at ? 'muted' : item.published ? 'live' : 'draft'}`}>{item.deleted_at ? 'حذف‌شده' : item.published ? 'منتشرشده' : 'پیش‌نویس'}</span></div></button>)}{items.length === 0 && <div className="empty">موردی برای نمایش نیست.</div>}</div><div className="pagination"><button className="button" disabled={pageIndex === 0} onClick={() => setPageIndex(pageIndex - 1)}>صفحهٔ قبل</button><span>صفحهٔ {pageIndex + 1}</span><button className="button" disabled={!hasNextPage} onClick={() => setPageIndex(pageIndex + 1)}>صفحهٔ بعد</button></div></section>
        <section className="editor-pane">{draft ? <><div className="editor-heading"><div><div className="eyebrow">{draft.id ? 'EDIT CONTENT' : 'NEW CONTENT'}</div><h2>{preview.title || preview.name || 'محتوای تازه'}</h2><small>{draft.id ? `نسخه ${draft.version}` : 'پیش‌نویس جدید'}</small></div><button className="close" onClick={() => { setDraft(null); setSelected(null); }}>×</button></div><div className="editor-tabs"><button className={tab === 'content' ? 'active' : ''} onClick={() => setTab('content')}>محتوا</button><button className={tab === 'links' ? 'active' : ''} onClick={() => setTab('links')}>پیوند و ترتیب</button><button className={tab === 'assets' ? 'active' : ''} onClick={() => setTab('assets')}>مدیا و جلد</button><button className={tab === 'preview' ? 'active' : ''} onClick={() => setTab('preview')}>پیش‌نمایش</button></div><div className="editor-body">{tab === 'content' && <><label>شناسه مسیر<input value={draft.slug || ''} disabled={!!draft.id} onChange={event => setDraft({ ...draft, slug: event.target.value })} placeholder="slug" /></label><label>ترتیب نمایش<input type="number" value={preview.displayOrder ?? ''} onChange={event => changePayloadField('displayOrder', event.target.value === '' ? null : Number(event.target.value))} placeholder="ترتیب پیش‌فرض" /></label><div className="editor-mode"><span>متن‌ها و مشخصات محتوا</span><button className="text-button" onClick={() => setShowRaw(!showRaw)}>{showRaw ? 'نمایش فرم' : 'ویرایش پیشرفتهٔ داده'}</button></div>{showRaw ? <label>دادهٔ ساختاریافته<textarea spellCheck="false" value={payloadText} onChange={event => setPayloadText(event.target.value)} /></label> : <><PayloadFields payload={preview} kind={draft.kind} onChange={changePayloadField} />{draft.kind === 'page' && draft.slug === 'home' && <HomeSections value={preview.sections} onChange={sections => changePayloadField('sections', sections)} />}</>}</>}{tab === 'links' && <RelationEditor draft={draft} setDraft={setDraft} allItems={allItems} />}{tab === 'assets' && <MediaEditor draft={draft} setDraft={setDraft} media={media} kind={kind} onUpload={uploadFile} />}{tab === 'preview' && <div className="preview-card"><div className="eyebrow">PREVIEW · DRAFT</div>{draft.media.filter(m => m.slot === 'cover' || m.slot === 'image').map(m => { const asset = media.find(a => a.id === m.mediaId); return asset?.public_url && <img key={m.mediaId} src={asset.public_url} alt={asset.alt_fa || preview.title || ''} />; })}<h2>{preview.title || preview.name || preview.label}</h2><p>{preview.lead || preview.short || preview.description || preview.summary || preview.text}</p><div className="preview-meta">{draft.relations.length} پیوند · {draft.media.length} فایل · فقط در پنل</div></div>}</div><div className="editor-actions"><button className="button primary" disabled={busy || !['owner','editor'].includes(user.role)} onClick={save}>ذخیرهٔ پیش‌نویس</button>{draft.id && <><button className="button" disabled={busy || !['owner','publisher'].includes(user.role)} onClick={() => action('publish')}>انتشار</button><button className="button subtle" disabled={busy || !['owner','publisher'].includes(user.role)} onClick={() => action('unpublish')}>لغو انتشار</button><button className="button danger" disabled={busy || !['owner','editor'].includes(user.role)} onClick={() => action(draft.deleted_at ? 'restore' : 'trash')}>{draft.deleted_at ? 'بازیابی' : 'انتقال به زباله‌دان'}</button></>}</div></> : <div className="editor-empty"><div className="monogram">نـ</div><h2>روایت را انتخاب کنید</h2><p>یک مورد از فهرست باز کنید یا محتوای تازه بسازید.</p></div>}</section>
      </div>}{message && <div className="toast" role="status">{message}<button onClick={() => setMessage('')}>×</button></div>}
    </main>
  </div>;
}

const homeSectionLabels = { heading: 'عنوان و انتخاب حال‌وهوا', scene: 'کارت‌های اصلی و ویترین آثار', panorama: 'دورنمای تجربه‌ها' };
function HomeSections({ value, onChange }) {
  const rows = Array.isArray(value) ? value : Object.keys(homeSectionLabels).map(key => ({ key, enabled: true }));
  function move(index, offset) {
    const target = index + offset;
    if (target < 0 || target >= rows.length) return;
    const next = [...rows]; [next[index], next[target]] = [next[target], next[index]]; onChange(next);
  }
  return <section className="home-sections"><h3>بخش‌های صفحهٔ خانه</h3><p className="field-help">نمایش و ترتیب بخش‌ها را از اینجا تنظیم کنید. متن و محتوای هر بخش در همین صفحه و تب پیوندها ویرایش می‌شود.</p>{rows.map((row, index) => <div className="home-section-row" key={row.key}><label className="checkbox-field"><input type="checkbox" checked={row.enabled !== false} onChange={event => onChange(rows.map((item, i) => i === index ? { ...item, enabled: event.target.checked } : item))} />{homeSectionLabels[row.key] || row.key}</label><button type="button" disabled={index === 0} onClick={() => move(index, -1)} aria-label="انتقال به بالا">↑</button><button type="button" disabled={index === rows.length - 1} onClick={() => move(index, 1)} aria-label="انتقال به پایین">↓</button></div>)}</section>;
}
...
function PayloadFields({ payload, kind, onChange }) {
  ...
  return <div className="payload-fields">{entries.map(([key, value]) => {
    ...
    if (typeof value === 'string') return <label key={key}>{label}{value.length > 100 || ['description','summary','text','lead','subtitle','bio'].includes(key) ? <textarea className="text-field" value={value} onChange={event => onChange(key, event.target.value)} /> : <input value={value} onChange={event => onChange(key, event.target.value)} />}</label>;
    return <div className="field-help" key={key}>{label}: برای تغییر این دادهٔ پیچیده، ویرایش پیشرفته را باز کنید.</div>;
  })}{optional.filter(key => !(key in (payload || {}))).map(key => <label key={key}>{fieldLabels[key]}<input value="" placeholder="برای افزودن مقدار وارد کنید" onChange={event => onChange(key, event.target.value)} /></label>)}</div>;
}

function RelationEditor({ draft, setDraft, allItems }) {
  const suggestedFields = draft.kind === 'page' && draft.slug === 'home'
    ? ['featured_experience','question_experience','audio_experience','featured_book','featured_world','panorama','showcase']
    : draft.kind === 'page' && draft.slug === 'share' ? ['works']
    : ({ book: ['author'], experience: ['book','primary_world','moods','words'], world: ['mood'], mood: ['world'], photo: ['book'], member_work: ['world'], collection: ['items'] })[draft.kind] || ['items'];
  const targetKinds = { author: 'author', book: 'book', primary_world: 'world', world: 'world',
    mood: 'mood', moods: 'mood', words: 'world', featured_experience: 'experience',
    question_experience: 'experience', audio_experience: 'experience', featured_book: 'book',
    featured_world: 'world', panorama: 'experience', showcase: 'member_work', works: 'member_work' };
  ...
  return <div><p className="field-help">ارتباط کتاب، تجربه، جهان و چیدمان Home از اینجا تعیین می‌شود. ترتیب هر گروه با پیکان تغییر می‌کند.</p>{draft.relations.map((relation, index) => <div className="relation-row" key={`${relation.field}-${index}`}><input list="relation-field-suggestions" value={relation.field} onChange={event => change(index, { field: event.target.value, targetId: '' })} placeholder="نوع پیوند" /><select value={relation.targetId} onChange={event => change(index, { targetId: event.target.value })}><option value="">انتخاب محتوا</option>{allItems.filter(item => !item.deleted_at && !item.archived_at && (!targetKinds[relation.field] || item.kind === targetKinds[relation.field])).map(item => <option key={item.id} value={item.id}>{item.kind} · {item.payload?.title || item.payload?.name || item.stable_key}</option>)}</select><input value={relation.label || ''} onChange={event => change(index, { label: event.target.value })} placeholder="برچسب" /><button onClick={() => move(index, -1)}>↑</button><button onClick={() => move(index, 1)}>↓</button><button onClick={() => setDraft({ ...draft, relations: draft.relations.filter((_, i) => i !== index) })}>×</button></div>)}<datalist id="relation-field-suggestions">{suggestedFields.map(field => <option key={field} value={field} />)}</datalist><button className="button" onClick={() => { const field = suggestedFields.find(name => !draft.relations.some(row => row.field === name)) || suggestedFields.at(-1); setDraft({ ...draft, relations: [...draft.relations, { field, ordinal: draft.relations.filter(r => r.field === field).length, targetId: '', label: '' }] }); }}>افزودن پیوند</button></div>;
}
...
function MediaEditor({ draft, setDraft, media, kind, onUpload }) {
  ...
  const add = (slot, mediaId = '') => setDraft(current => ({ ...current, media: [...current.media, { slot, ordinal: current.media.filter(m => m.slot === slot).length, mediaId, crop: {} }] }));
  const attachUploaded = (slot, mediaId) => setDraft(current => {
    const index = current.media.findIndex(reference => reference.slot === slot);
    return { ...current, media: index < 0 ? [...current.media, { slot, ordinal: 0, mediaId, crop: {} }] : current.media.map((reference, i) => i === index ? { ...reference, mediaId } : reference) };
  });
  return <div className="media-editor"><p className="field-help">فایل را از سیستم انتخاب کنید یا از کتابخانهٔ مدیا وصل کنید. پس از تغییر، «ذخیرهٔ پیش‌نویس» و سپس «انتشار» را بزنید. برداشتن اتصال، فایل قدیمی را از کتابخانه پاک نمی‌کند.</p>
    <label className="button upload-button">انتخاب و آپلود از سیستم<input type="file" accept={accept(defaultSlot)} onChange={async event => { const file = event.target.files?.[0]; if (file) await onUpload(file, asset => attachUploaded(defaultSlot, asset.id)); event.target.value = ''; }} /></label>
    {draft.media.map((reference, index) => {
      const current = media.find(asset => asset.id === reference.mediaId);
      return <div className="media-reference" key={index}><select value={reference.slot} onChange={event => change(index, { slot: event.target.value, mediaId: '' })}>{slots.map(slot => <option key={slot} value={slot}>{slot}</option>)}</select><select value={reference.mediaId} onChange={event => change(index, { mediaId: event.target.value })}><option value="">انتخاب فایل</option>{media.filter(asset => matches(asset, reference.slot)).map(asset => <option key={asset.id} value={asset.id}>{asset.object_key.split('/').pop()} · {asset.mime_type}</option>)}</select><label className="button upload-button">بارگذاری جایگزین<input type="file" accept={accept(reference.slot)} onChange={async event => { const file = event.target.files?.[0]; if (file) await onUpload(file, asset => change(index, { mediaId: asset.id })); event.target.value = ''; }} /></label>{current?.public_url && current.mime_type.startsWith('image/') && <img src={current.public_url} alt="پیش‌نمایش فایل" />}{current?.public_url && current.mime_type.startsWith('audio/') && <audio controls preload="none" src={current.public_url} />}{current?.public_url && current.mime_type.startsWith('video/') && <video controls preload="none" src={current.public_url} />}<button type="button" onClick={() => setDraft(current => ({ ...current, media: current.media.filter((_, i) => i !== index) }))} aria-label="حذف اتصال">×</button></div>;
    })}
    <button className="button" onClick={() => add(defaultSlot)}>اتصال فایل دیگر</button>
    {(kind === 'podcast' || kind === 'experience') && <p className="field-help">برای نشانی مستقیم فایل یا امبد Spotify و SoundCloud، در تب «محتوا» فیلد صوت را وارد کنید. فایل آپلودی در اولویت نمایش است.</p>}
  </div>;
}
